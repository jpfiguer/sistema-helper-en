 ...
 *   npm run reporte
 *
 * Lee los JSONL de `sesiones/` y muestra cómo se movieron las métricas en el tiempo. Es la
 * única parte del sistema que contesta la pregunta que importa de verdad — "¿estoy mejorando
 * o me lo estoy imaginando?" — y por eso no usa el LLM en ningún punto: si el juez de tu
 * progreso puede alucinar, el progreso no se puede leer.
 *
 * Una sesión suelta no dice nada: el ruido entre días (cansancio, tema de las preguntas, hora)
 * es más grande que la mejora semanal. Por eso el reporte muestra la serie completa y una
 * comparación entre las primeras y las últimas tres sesiones, que es donde la señal aparece.
 *
 * Opciones:
 *   --n=20        cuántas sesiones mostrar (por defecto 15)
...
function main() {
  ...
  const ult = todas.slice(-n);

  console.log(`\n${NEG}Progreso — ${todas.length} sesión${todas.length > 1 ? 'es' : ''}${FIN}${todas.length > ult.length ? `${GRIS} (últimas ${ult.length})${FIN}` : ''}\n`);
  ...
  if (todas.length >= 4) {
    const k = Math.min(3, Math.floor(todas.length / 2));
    const prom = (lista, sel) => {
      const v = lista.map(sel).filter((x) => typeof x === 'number');
      return v.length ? v.reduce((a, b) => a + b, 0) / v.length : null;
    };
    const ini = todas.slice(0, k);
    const fin = todas.slice(-k);
    ...
    console.log(`\n${GRIS}  Ojo con leer de más en una sola sesión: el ruido entre días es mayor que la mejora\n  semanal. La serie importa, el punto suelto no.${FIN}`);
  } else {
    console.log(`\n${AMAR}  Con ${todas.length} sesión${todas.length > 1 ? 'es' : ''} todavía no hay tendencia que leer.${FIN}`);
